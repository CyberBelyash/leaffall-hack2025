...
# ----------------------------
# OSRM: получение реального расстояния с кэшированием
# ----------------------------
...
# ----------------------------
# 4. REINFORCE с OSRM-расстоянием
# ----------------------------
...
print(f"🟢 Жадное время (haversine): {optimal_time:.1f} мин")
...
print("🚀 Начало обучения с учётом временных окон и OSRM-расстояний...")
...
# ----------------------------
def get_route(model, data, coords, osrm_dist_matrix, work_start, work_end, lunch_start, lunch_end, is_vip, speed_kmh=30.0):
    N = data.x.size(0)
    visited = torch.zeros(N, dtype=torch.bool)
    current = 0
    visited[current] = True
...
# ----------------------------
def get_route_onnx(ort_session, data, coords, osrm_dist_matrix, work_start, work_end, lunch_start, lunch_end, is_vip, speed_kmh=30.0):
    N = data.x.size(0)
...
# Сохраняем карту для ONNX-маршрута
...
plt.axhline(y=optimal_time, color='green', linestyle='--', linewidth=2, label=f'Greedy (haversine): {optimal_time:.1f} min')
