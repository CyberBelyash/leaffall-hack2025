...
from scipy.spatial import cKDTree  # <-- добавлено
...
# ----------------------------
# Нормализация координат в км относительно центра
# ----------------------------
...
lat_km = (lats - lat_center) * 111.32
lon_km = (lons - lon_center) * 111.32 * np.cos(np.radians(lat_center))

# Обновляем DataFrame и признаки
df['lat_norm'] = lat_km
df['lon_norm'] = lon_km

node_features = df[[
    'lat_norm', 'lon_norm',  # нормализованные координаты
    'is_vip', 'work_start', 'work_end', 'lunch_start', 'lunch_end'
]].values
x = torch.tensor(node_features, dtype=torch.float)
N = len(df)
coords = list(zip(df['Географическая широта'], df['Географическая долгота']))  # исходные для OSRM/haversine
...
def reinforce_batch_step(model, data, coords, osrm_dist_matrix, work_start, work_end, lunch_start, lunch_end, is_vip,
    ...
    for _ in range(num_samples):
        ...
        current = random.randint(0, N - 1)  # при обучении — случайный старт для exploration
...
# ----------------------------
# 9. Определение стартовой точки через cKDTree
# ----------------------------
coords_km = np.column_stack((lat_km, lon_km))  # (N, 2)
tree = cKDTree(coords_km)
_, start_idx = tree.query([0.0, 0.0])  # центр — (0,0) в нормализованных км
print(f"🎯 Стартовая точка (ближайшая к центру): узел {start_idx}")

# ----------------------------
# 10. Запуск маршрутизации
# ----------------------------
final_route, final_time = get_route(
    model, data, coords, osrm_dist_matrix, work_start, work_end,
    lunch_start, lunch_end, is_vip,
    speed_kmh=30.0, service_time_min=30.0,
...
# ----------------------------
# 12. Визуализация с OSRM-геометрией
# ----------------------------
