...
def get_route_onnx_inference(graph_data, speed_kmh=30.0, service_time_min=30.0):
    ...
    for step in range(N - 1):
        ...
        # Прибыли рано — ждём начала рабочего дня клиента
        if arrival_time < ws:
            arrival_time = ws

        # Примечание: нарушения (прибытие после work_end или в обед) не прерывают маршрут,
        # но время всё равно учитывается. В продакшене можно добавить фильтрацию.
...
async def get_route(request: RouteRequest):
    try:
        ...
        return RouteResponse(route=route, total_time_minutes=round(total_time, 2))
    except Exception as e:
